...
const podo_mainMenuPC = {
    init() {
        const gnbMenu = document.querySelector(".podo-main-menu:not(.sample) .gnb-menu");
    ...
    resetMainMenu() {
        document.querySelectorAll(".podo-main-menu:not(.sample) .gnb-main-trigger:not(.is-link)").forEach((mainTrigger) => {
            mainTrigger.classList.remove("active");
            mainTrigger.setAttribute("aria-expanded", "false");
        });
        document.querySelectorAll(".podo-main-menu:not(.sample) .gnb-toggle-wrap").forEach((toggleWrap) => {
    ...
    attachEvents(mainTriggers, subTriggers) {
        // podo-main-menu 외부 클릭시 닫기
        document.addEventListener("click", ({ target }) => {
            if (!target.closest(".podo-main-menu")) this.closeMainMenu();
        });
        ...
        document.addEventListener("keyup", (event) => {
            if (event.code === "Escape" || !event.target.closest(".podo-main-menu")) {
                this.closeMainMenu();
            }
        });
...
const podo_mainMenuMobile = {
    init() {
        const mobileGnb = document.querySelector(".podo-main-menu-mobile:not(.sample)");
    ...
    openMainMenu(mobileGnb) {
        ...
        // const id = mobileGnb.getAttribute("id");
        // const openGnb = document.querySelector(`[aria-controls=${id}]`);
        // const header = document.querySelector("#podo-header");
        ...
        mobileGnb.addEventListener("transitionend", function onTransitionEnd() {
            ...
            // inert 설정
            document.querySelector("#podo-header .header-in").setAttribute("inert", "");
    ...
    closeMainMenu(mobileGnb) {
        ...
        // inert 설정
        document.querySelector("#podo-header .header-in").removeAttribute("inert");
    ...
    resetAnchorMenu() {
        document.querySelectorAll(".podo-main-menu-mobile .menu-wrap .gnb-main-trigger").forEach((menu) => {
...
const podo_sideNavigation = {
    ...
    setupSideNavLists() {
        const sideNavLists = document.querySelectorAll(".podo-side-navigation .lnb-list");
    ...
    setupToggleEvents() {
        const toggleButtons = document.querySelectorAll(".podo-side-navigation .lnb-list:not(.exception-case) .lnb-toggle");
    ...
    setActiveCurrentPage() {
        ...
        const lnbLinks = document.querySelectorAll(".podo-side-navigation .lnb-link");
...
const podo_tab = {
    layerTabArea: null,
    init() {
        this.layerTabArea = document.querySelectorAll(".podo-tab-area.layer");
            ...
            layerTabs.forEach((tab) => {
                ...
                if (!tab.dataset.listenerAttached) {
                    ...
                    tab.addEventListener("click", () => {
                        const closestTabs = tab.closest(".podo-tab-area.layer > .tab").querySelectorAll("li");
                        const closestTabPanels = tab.closest(".podo-tab-area.layer").querySelectorAll(":scope > .tab-conts-wrap > .tab-conts");
...
const podo_accordion = {
    ...
    setupAccordions() {
        this.accordionButtons.forEach((button, idx) => {
            const accordionContainer = button.closest(".podo-accordion");
...
const podo_modal = {
    ...
    setupTriggers() {
        ...
        this.modalCloseTriggers.forEach((trigger) => {
            trigger.addEventListener("click", (event) => {
                event.preventDefault();
                const modalId = trigger.closest(".podo-modal").getAttribute("id");
        ...
        dialogElement.addEventListener(
            ...
            (event) => {
                if (event.key === "Escape" || event.key === "Esc") {
                    this.closeModal(dialogElement.closest(".podo-modal").id);
                }
            },
            { once: true }
        );
...
/*** * podo_contextualHelp * ***/
const podo_contextualHelp = {
    tooltipButtons: null,
    init() {
        this.tooltipButtons = document.querySelectorAll(".podo-contextual-help .tooltip-btn");
    ...
    setupTooltips() {
        this.tooltipButtons.forEach((button) => {
            const tooltipContainer = button.closest(".podo-contextual-help");
    ...
    closeAllTooltips() {
        const otherPopovers = document.querySelectorAll(".podo-contextual-help .tooltip-popover");
    ...
    adjustTooltipPosition(tooltipContainer, tooltipPopover) {
        ...
        if (isMobile) {
            ...
            const contentsPaddingX = rootStyles.getPropertyValue("--podo-contents-padding-x").trim().split("px")[0];
...
const podo_tooltip = {
    ...
    isMobile: null,
    init() {
        this.tooltip = document.querySelectorAll(".podo-tooltip");
    ...
    createTooltipPopover(uniqueIdx, tooltipBtnText, tooltipText) {
        ...
        tooltipPopover.classList.add("podo-tooltip-popover");
    ...
    closeAllTooltips() {
        const otherPopovers = document.querySelectorAll(".podo-tooltip-popover");
        otherPopovers.forEach((popover) => {
            if (!popover.classList.contains("active")) return;
            popover.style = "";
            popover.className = "podo-tooltip-popover";
        });
    },
...
/*** * podo_calendar * ***/ // *btn-set-date: aria-pressed > aria-selected 변경 해야함(상위 속성도 같이)
const podo_calendar = {
    datePickerArea: null,
    activeDateSelector: null,
    init() {
        this.datePickerArea = document.querySelectorAll(".podo-calendar-area");
    ...
    setupOpenCloseEvents() {
        ...
        this.datePickerArea.forEach((datePicker) => {
            ...
            // const lastActionButton = datePicker.querySelector(".calendar-btn-wrap .podo-btn:last-child");
            // if (lastActionButton) {
    ...
    openDatePicker(button) {
        ...
        const currentDatePicker = button.closest(".calendar-conts").querySelector(".podo-calendar-area");
            ...
            const accReset = (action, btn, type) => {
                ...
                action.addEventListener("click", () => {
                    if (targetInput) {
                        ...
                        if (target === "확인") {
                            if (type === "single") {
                                const value = action.closest(".podo-calendar-area").querySelector("td.period.start.end").getAttribute("data-date");
                                targetInput.value = value;
                            } else {
                                const value1 = action.closest(".podo-calendar-area").querySelector("td.period.start")?.getAttribute("data-date");
                                const value2 = action.closest(".podo-calendar-area").querySelector("td.period.end")?.getAttribute("data-date") || "";
                ...
                // 공통 접근성
                const accSet = () => {
                    const prevItems = action.closest(".podo-calendar-area").querySelectorAll(".period");
                    prevItems.forEach((prev) => {
                        prev.classList.remove("period", "start", "end");
                        prev.querySelector(".btn-set-date").removeAttribute("aria-pressed");
                    });
                    action.closest(".podo-calendar-area").querySelector("td.today").classList.add("period", "start", "end");
                    action.closest(".podo-calendar-area").querySelector("td.today .btn-set-date").setAttribute("aria-pressed", "true");
                };
            };
...
/*** * podo_inPageNavigation * ***/
const podo_inPageNavigation = {
    quickIndicators: null,
    init() {
        this.quickIndicators = document.querySelectorAll(".podo-in-page-navigation-type .podo-in-page-navigation-area:not(.sample) .in-page-navigation-list");
    ...
    observeListChanges() {
        ...
        const quickList = document.querySelector(".podo-in-page-navigation-type .in-page-navigation-list");
        if (!quickList) return;
    ...
    calculateHeaderHeight() {
        const headerTop = document.querySelector("#podo-masthead")?.clientHeight || 0;
        const headerInner = document.querySelector("#podo-header .header-in")?.clientHeight || 0;
    ...
    updateActiveSection() {
        ...
        if (sectionArea.length > 0) {
            ...
            sectionArea.forEach((current) => {
                ...
                const navLink = document.querySelector(`.podo-in-page-navigation-area a[href*=${sectionId}]`);
                const firstAnchor = document.querySelector(".podo-in-page-navigation-area .in-page-navigation-list li:first-of-type a");
                const lastAnchor = document.querySelector(".podo-in-page-navigation-area .in-page-navigation-list li:last-of-type a");
...
/*** * podo_helpPanel * ***/
const podo_helpPanel = {
    helpPanel: null,
    lastFocusedButton: null,
    executeButton: null,
    collapseButton: null,
    init() {
        this.helpPanel = document.querySelector(".podo-help-panel");
    ...
    setupPadding() {
        const topBannerHeight = document.querySelector("#podo-masthead")?.offsetHeight;
        const headerHeight = document.querySelector("#podo-header .header-in")?.offsetHeight;
    ...
    observeMastHead() {
        const topBanner = document.querySelector("#podo-masthead");
        if (!topBanner) return;
...
/*** * podo_disclosure * ***/
const podo_disclosure = {
    disclosures: null,
    init() {
        this.disclosures = document.querySelectorAll(".podo-disclosure");
...
/*** * podo_adjustContentScale * ***/
const podo_adjustContentScale = {
    body: document.body,
    scaleLevel: 1,
    minScale: 0.5,
    maxScale: 2,
    init() {
        ...
        const zoomSmall = rootStyles.getPropertyValue("--podo-zoom-small").trim();
        const zoomMedium = rootStyles.getPropertyValue("--podo-zoom-medium").trim();
        const zoomLarge = rootStyles.getPropertyValue("--podo-zoom-large").trim();
        const zoomXlarge = rootStyles.getPropertyValue("--podo-zoom-xlarge").trim();
        const zoomXxlarge = rootStyles.getPropertyValue("--podo-zoom-xxlarge").trim();
...
/*** * podo_toggleSwitch * ***/
const podo_toggleSwitch = {
    init() {
        const toggleSwitch = document.querySelectorAll(".podo-form-toggle-switch");
...
const podo_infoList = {
    init() {
        const infoLists = document.querySelectorAll(".podo-info-list");
...
/*** * podo_dropEvent(gnb utils / page-title-wrap) * ***/
const podo_dropEvent = {
    dropButtons: null,
    init() {
        this.dropButtons = document.querySelectorAll(".podo-drop-wrap:not(.sample) .drop-btn");
    ...
    setupMenuItems(menu) {
        ...
        items.forEach((item) => {
            ...
            item.addEventListener("click", () => {
                ...
                this.closeAllDropdowns();
                const button = item.closest(".podo-drop-wrap").querySelector(".drop-btn");
                button?.focus();
            });

            item.addEventListener("focus", () => {
                document.querySelectorAll(".podo-drop-wrap .drop-list .item-link").forEach((item) => {
    ...
    openDropdown(button, menu) {
        ...
        if (menuRect.left < 0) {
            menu.closest(".podo-drop-wrap").classList.add("drop-left");
        } else if (windowWidth < menuRect.left + menuRect.width) {
            menu.closest(".podo-drop-wrap").classList.add("drop-right");
        }

    },
    closeAllDropdowns() {
        document.querySelectorAll(".podo-drop-wrap .drop-menu").forEach((menu) => {
    ...
    setupFocusOutEvent() {
        ...
        document.addEventListener("keydown", (event) => {
            if (event.key === "Escape" || event.key === "Esc") {
                this.closeAllDropdowns();
                event.target.closest(".podo-drop-wrap")?.querySelector(".drop-btn")?.focus();
            }
        });

        // 드롭다운 외부 클릭 시 닫기
        document.addEventListener("click", (event) => {
            if (!event.target.closest(".podo-drop-wrap")) {
                this.closeAllDropdowns();
            }
        });
...
/*** * podo_chkBox 박스형 체크박스 상태에 따른 디자인 변경 * ***/
const podo_chkBox = {
    init() {
    ...
    formChipFocus() {
        // form_chip 일때 포커스 처리
        const formChip = document.querySelectorAll(".podo-form-chip");
...
/*** * podo_fileUpload 파일 업로드 : drag 임시 * ***/
const podo_fileUpload = {
    init() {
...
const nuriToggleEvent = {
    init() {
        const _toggleBtns = document.querySelectorAll("#podo-masthead .toggle-btn");
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    podo_sideNavigation.init();
    podo_tab.init();
    podo_accordion.init();
    podo_modal.init();
    podo_contextualHelp.init();
    podo_tooltip.init();
    podo_disclosure.init();
    podo_dropEvent.init();
    podo_calendar.init();
    podo_inPageNavigation.init();
    podo_adjustContentScale.init();
    podo_toggleSwitch.init();
    podo_infoList.init();
    podo_chkBox.init();
    podo_fileUpload.init();

    podo_helpPanel.init();
    if (windowSize.getWinSize() === "pc") {
        podo_helpPanel.toggleHelpPanel("open");
    }
});
...
window.addEventListener("scroll", () => {
    ...
    podo_mainMenuPC.closeMainMenu();
    podo_inPageNavigation.updateActiveSection();
    podo_helpPanel.init();
});
